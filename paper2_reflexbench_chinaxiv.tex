...
% ====== NeurIPS 2026 Style ======
\usepackage[preprint]{neurips_2026}  % ChinaXiv preprint version (non-anonymous)
...
This is the \textbf{observer-invariance assumption}, formalized in prior work~\citep{zhang2026}: an environment is observer-invariant if its dynamics are independent of the agent. Every benchmark listed above inhabits an observer-invariant environment. Every major AI breakthrough---AlphaGo, GPT-4, AlphaFold, DeepSeek-R1---operates in one.
...
\item \textbf{Empirical evaluation} of 7 frontier LLMs across 5 providers, revealing systematic reflexivity gaps in state-of-the-art systems (\S\ref{sec:experiments}).
...
Following the framework in~\citep{zhang2026}, we define:
...
All models were evaluated under identical zero-shot conditions across all 20 scenarios (80 evaluation points per model, 400 total). Responses were collected in April 2026.
...
\caption{Observer Depth profiles for all 7 evaluated models. Each cluster shows scores at four OD levels (0--$n$). All models exhibit monotonic degradation from OD-0 to OD-$n$, with $\Delta$ shown in red. Reasoning models (Claude, DeepSeek, Gemini, Kimi) show flatter degradation profiles than generic models.}
\label{fig:od-profile}
\end{figure}

\begin{table}[h]
\centering
\caption{ReflexBench aggregate results: mean scores across 20 scenarios (0--1 per Part, 0--4 total), scored via two-stage protocol (\S\ref{sec:od}). 95\% bootstrap CI for top models. \textbf{Bold}: best per column.}
\label{tab:results}
...
\textbf{Observation 3: The gap persists across model families.} With 7 models spanning 7 different providers (Anthropic, DeepSeek, Google, Moonshot, Zhipu, ByteDance, Alibaba), the universal degradation ($\Delta \approx -0.47$) confirms the theoretical prediction~\citep{zhang2026}: pretraining on observer-invariant data cannot close the reflexivity gap. This is a structural limitation, not a capacity limitation.
...
The quantitative degradation conceals revealing qualitative patterns. We analyze all 77 OD-$n$ failures across 7 models (11/20 scenarios fail for Gemini 2.5 Pro alone) and identify four distinct failure modes:
...
\begin{table}[h]
\centering
\caption{Failure mode distribution across 7 models at OD-2+ (77 total failures).}
...
A natural question is whether OD scores are simply proxies for general intelligence. We compute Pearson correlations between OD levels across all 7 models $\times$ 20 scenarios:
...
The Ouroboros system~\citep{zhang2026} was trained through 14 iterative rounds of multi-reward GRPO on a 35B-parameter MoE base model. The training used 10 tier-level reward dimensions (50 subdimensions) organized in a 9-tier cognitively-motivated architecture~\citep{zhang2026}:
...
The phase transition results connect to a fundamental constraint derived in prior work~\citep{zhang2026}:
...
\item \textbf{Model coverage}: We tested 7 frontier models across 5 providers. Expanding to additional models (e.g., GPT-4o) and domain-specific fine-tuned models would further strengthen generalizability.
...
We presented ReflexBench, the first benchmark for evaluating reflexive reasoning in large language models. Our evaluation of 7 models across 20 scenarios reveals that:
...
The benchmark, evaluation protocol, scoring code, and complete model responses are publicly available.\footnote{An earlier version of this paper was deposited on Zenodo (DOI: 10.5281/zenodo.19627242). The present version extends the evaluation to 7 models and incorporates V20.1 training evidence. Repository: \url{https://github.com/mmjbds/reflexbench}}

% ====================================================================
\section*{Acknowledgments}
% ====================================================================

The author used large language models for editing assistance, literature search, and prose organization during the preparation of this manuscript. All benchmark design, experimental execution, reward function engineering, and analysis reflect the author's original work. The Ouroboros training pipeline was executed on AutoDL cloud infrastructure (2$\times$A800 80GB).
...
\textit{Zenodo Preprint}. DOI: 10.5281/zenodo.19557261.
