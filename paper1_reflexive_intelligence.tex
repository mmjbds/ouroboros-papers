...
We provide evidence from a financial world model, where a 3B-active-parameter Mixture-of-Experts model---trained across 14 iterative rounds at a cost accessible to individual researchers---exhibits emergent reflexive reasoning not spontaneously produced by generic models with $600\times$ more parameters under zero-shot conditions. The bottleneck, we find, is not scale but \textbf{training methodology}: the specific curriculum of failures survived and lessons encoded.
\end{abstract}
...
\item \textbf{Craft Knowledge Documentation}: the first systematic account of multi-reward GRPO interaction effects, stabilization patterns, and failure modes across 14 iterative training rounds---tacit knowledge absent from the algorithmic literature (\S5.5).
\item \textbf{Empirical Evidence} that a 3B-active model trained with this framework exhibits reflexive reasoning not spontaneously produced by generic models with vastly more parameters (\S6).
\end{enumerate}
...
\subsection{Observer Depth: A Taxonomy of Reflexive Reasoning}

Reflexive intelligence is not binary. We propose \textbf{Observer Depth (OD)} as a quantitative measure of reflexive reasoning complexity:

\vspace{0.3em}
\noindent\textbf{Definition 4 (Observer Depth).} \textit{The recursive depth at which an agent models its own causal impact on the environment.}
\vspace{0.3em}

\begin{itemize}[leftmargin=*]
\item \textbf{OD-0}: $D_0(x) = f(x)$. The agent analyzes the environment without considering its own presence. All current LLMs, including GPT-4o and GPT-5.4, operate at OD-0 under zero-shot conditions.
\item \textbf{OD-1}: $D_1(x) = f(x, \Phi(a))$. The agent considers its own action's immediate impact. \textit{``My large buy order will push the price up.''}
\item \textbf{OD-2}: $D_2(x) = f(x, \Phi(a), \Psi_{\text{others}}(\Phi(a)))$. The agent models how other participants react to its impact. \textit{``My buy pushes the price up; other algorithms will detect this and follow; the combined impact exceeds my estimate.''}
\item \textbf{OD-$n$}: Recursive modeling to depth $n$, with $\text{OD-}\infty$ corresponding to a reflexive fixed point $\pi^* = T(\pi^*)$ where the strategy remains optimal even when fully transparent.
\end{itemize}

We conjecture that each level of Observer Depth represents a qualitatively distinct cognitive capability, and that transitions between levels exhibit phase-transition-like behavior during training (see \S6).

\subsection{The Reflexive Uncertainty Principle}

We derive a fundamental constraint on reflexive agents, analogous in structure---though not in mechanism---to the Heisenberg uncertainty principle:

\vspace{0.3em}
\noindent\textbf{Theorem 1 (Reflexive Uncertainty Principle).} \textit{For any agent operating in an observer-participant environment with finite liquidity, the product of prediction accuracy $\Delta P$ and action impact $\Delta I$ is bounded below:}
\begin{equation}
\Delta P \cdot \Delta I \geq \Omega
\end{equation}
\textit{where} $\Omega = f(\text{position\_size}, \text{market\_depth})$ \textit{is the reflexive constant of the environment.}
\vspace{0.3em}

\textbf{Proof sketch.} When an agent acts on a prediction with confidence $P$, it takes a position proportional to $P$. This position impacts the market by $I \propto P$. The impact degrades the prediction's accuracy to $P_{\text{eff}} = P(1-kI)$, yielding $\Delta P = kP^2$ and $\Delta I = P$, so $\Delta P \cdot \Delta I = kP^3 \geq \Omega$ for any non-trivial position.

The implications are profound:
\begin{enumerate}[leftmargin=*]
\item \textbf{No system---regardless of scale---can violate this bound.} A 10-trillion-parameter model faces the same constraint as a 3B model.
\item \textbf{The optimal strategy is not maximal accuracy, but optimal position within the uncertainty bound.} Agents that recognize this outperform agents that blindly maximize prediction accuracy.
\item \textbf{$\Omega$ decreases with market depth.} This explains why reflexive effects dominate in illiquid markets and diminish in deep ones.
\end{enumerate}
...
Phase 4: + World Model / Meta-cognition rewards $\to$ the model learns higher-order strategic reasoning
\end{quote}

Children learn language before logic, logic before empathy. The training curriculum follows the same progression---not by design, but because we discovered through repeated failure that any other order collapses. Across 14 rounds, the reward architecture grew from 2 sub-rewards (V12) to 50 sub-rewards organized in 9 tiers (V20.1), with each expansion preceded by the stabilization of the preceding layer.
...
Across fourteen rounds, we documented the following interaction effects:
...
\item \textbf{Training}: GRPO with 10 tier-level reward dimensions (50 subdimensions), progressive activation across 4 phases and 14 iterative rounds (V12--V20.1)
\item \textbf{Architecture}: 9-tier reward system (SCRGNDWMT) with cross-tier resonance, antagonism, and adaptive resonance temperature
\item \textbf{Hardware}: $2\times$ NVIDIA A800 80GB GPUs
\item \textbf{Total cost}: Approximately \$400 in cloud GPU rental across 14 iterative training rounds
...
\textbf{Observation 1: Reflexive reasoning emerges without explicit supervision.} After fourteen rounds of iterative GRPO training, the model spontaneously generates text referencing reflexive dynamics, crowding effects, and regime-dependent position adjustment. \textbf{No supervised examples of reflexive reasoning were present in the training data.} The training data contained only market scenarios and format templates; reflexive behavior was incentivized purely through reward signals, not demonstrated through examples. This constitutes evidence that reflexive reasoning is a \textbf{learnable cognitive skill that can emerge from reinforcement learning alone}, rather than a capability that must be distilled from larger models.
...
\textbf{Observation 2: Reflexive emergence exhibits phase-transition behavior.} Across training rounds V12 through V17.5 (over 200 cumulative GRPO steps), the reflexivity awareness reward remained at exactly zero---the model produced no reflexive reasoning whatsoever. At Step 25 of the seventh round (V17.6), following progressive activation of reflexive reward functions at Step 20, the reflexivity score discontinuously rose to 0.013 and sustained across subsequent steps (0.013, 0.013, 0.006 at Steps 25--27). This discontinuous emergence---total absence followed by sustained presence---is characteristic of a \textbf{phase transition} rather than gradual learning.

We characterize this transition using the \textbf{Reward Pareto Phase Transition}: at the critical point, the interaction between structural rewards (format) and reflexive rewards shifts from \textit{competitive} ($\text{Cov}(r_{\text{format}}, r_{\text{reflex}}) < 0$) to \textit{complementary} ($\text{Cov} > 0$). Before the transition, improving format quality suppressed reflexive reasoning; after, the two capabilities became mutually reinforcing.

\textbf{Observation 3: Generic large models do not spontaneously exhibit reflexive reasoning.} GPT-4o (est.\ 1.8T parameters), when prompted with identical scenarios under zero-shot conditions, produces competent financial commentary but operates at \textbf{OD-0}: it does not reason about its own market impact. We note that this comparison is between a fine-tuned model and a zero-shot baseline---it demonstrates that reflexive training \textit{adds} capabilities that pretraining alone does not provide, not that scale is irrelevant. A fair comparison against GPT-4o with equivalent fine-tuning remains future work.

\textbf{Observation 4: Training methodology matters.} Fourteen iterative rounds, each incorporating 3--5 specific lessons from the previous round's documented failures, produced capabilities not present in the base model. The reward architecture itself evolved from 2 sub-rewards (V12) to 50 sub-rewards in 9 tiers (V20.1), with each expansion producing emergent capabilities absent from simpler configurations.

\textbf{Observation 5: Multi-reward interactions produce phase transitions.} We documented 20+ specific interaction effects across 14 rounds, including gradient dilution from competing objectives, mode collapse from temperature boundaries, and spontaneous emergence of reasoning structures. In V20.1, the resonance temperature $\tau$ underwent an autonomous transition from 3.0 to 5.0 at Step 31, and the emergence count peaked at 32---a 60\% increase over the prior generation---demonstrating that richer reward topologies generate richer undesigned capability integrations.

\textbf{Observation 6: Multi-tier synchronous activation as phase transition precursor.} At V20.1 Step 62, four tiers (G, M, T, W) simultaneously reached historical peak activation. This event was not observed in any of the 13 prior training rounds and represents the first evidence of cross-tier phase coherence.
...
\begin{itemize}[leftmargin=*]
\item \textbf{Observer Depth (OD)}: a formal taxonomy quantifying reflexive reasoning from OD-0 (no reflexivity) through OD-$\infty$ (reflexive fixed point).
\item \textbf{The Reflexive Uncertainty Principle} ($\Delta P \cdot \Delta I \geq \Omega$): a fundamental bound showing that prediction accuracy and action impact cannot be simultaneously optimized---establishing a physical limit that no system, regardless of scale, can violate.
\item \textbf{Reward Pareto Phase Transition}: the empirical discovery that reflexive capabilities emerge discontinuously when competing reward dimensions shift from competitive to complementary interaction.
\end{itemize}

Our neuroscience-motivated cognitive architecture and empirical evidence suggest that reflexive reasoning can emerge from a well-designed training curriculum in a model with 3B active parameters, at a cost accessible to individual researchers. The bottleneck is not compute. It is not parameters. It is the accumulated craft knowledge required to navigate the interaction space of multi-reward reinforcement learning---and the willingness to document fourteen rounds of failure before finding what works.

The observation that reflexive reasoning \textit{emerges spontaneously} from reinforcement learning---without any supervised examples of reflexive behavior---suggests that reflexivity is not merely a pattern to be memorized but a \textbf{cognitive capability that can be induced through appropriate training pressure}. This has implications well beyond financial markets: any domain where the agent's actions alter the environment it analyzes---policy-making, recommendation, epidemiology---may benefit from the same training methodology.
